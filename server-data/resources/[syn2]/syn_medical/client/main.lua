    ...
    while true do
        ...
        for k,v in pairs(Config.MedicLocations) do
            ...
            if distance <= 1.5 then
                ...
                if IsControlJustReleased(0, 0xD9D0E1C0) then
                    TriggerServerEvent('syn_medical:getjob', 'medic_house')
                end
            end
        end
        ...
        elseif WarMenu.IsMenuOpened('options_house') then
            if WarMenu.Button(_U('take_items')) then
                TriggerServerEvent('syn_medical:takeItems')
            end
            WarMenu.Display()
        ...
        elseif IsControlJustReleased(0, 5) then -- SUPR
            --TriggerServerEvent('syn_medical:getjob', 'medic')
        end
        Citizen.Wait(0)
    end
end)

RegisterNetEvent('syn_medical:auth')
AddEventHandler('syn_medical:auth', function(type)
    WarMenu.OpenMenu(type)
end)

RegisterNetEvent('syn_medical:open')
AddEventHandler('syn_medical:open', function(cb)
	WarMenu.OpenMenu('medic')
	--print ("openmenu")
end)


RegisterNetEvent('syn_medical:revive')
AddEventHandler('syn_medical:revive', function(closestPlayer)
    DoScreenFadeOut(800)
...
RegisterNetEvent('syn_medical:heal')
AddEventHandler('syn_medical:heal', function(closestPlayer)
